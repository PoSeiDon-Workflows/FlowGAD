...
def run(config: dict):
    ...
    class GCN(torch.nn.Module):
        def __init__(self, hidden_channels, drop_rate):
    ...
    model = GCN(hidden_channels=int(config["hidden"]), drop_rate = config["dropout"] ).float().to(device)
...
def problem():
    ...
    problem.add_hyperparameter((0.001, 1, "log-uniform"), "dropout")
